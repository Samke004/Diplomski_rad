...
def _visualise(model_name, inference_results, out_dir, n_show=8, seed=None):
    """
    Sprema kvalitativnu vizualizaciju (GT / Pred / Overlay) za n_show
    NASUMIČNO odabranih slika iz test seta.

    Odabir je reproducibilan (fiksni seed, default cfg.RANDOM_SEED) i
    NAMJERNO isti za svaki poziv s istim seedom - tako je isti skup
    slika prikazan za sve modele, sto omogucuje direktnu usporedbu
    kako razliciti modeli rade na istim primjerima.
    """
    ...
    n_show = min(n_show, len(inference_results))
    indices = rng.sample(range(len(inference_results)), n_show)
    indices.sort()  # zadrzi stabilan redoslijed prikaza (lakse za citanje)
    samples = [inference_results[i] for i in indices]
    n = len(samples)
    ...
    fig.suptitle(f"{model_name} — Multi-class Results (random sample, seed={seed})\n"
