...
class DepthEncoder(nn.Module):
    ...
    def __init__(self, pretrained: bool = True):
        ...
        # HHA ima 3 kanala kao RGB — koristimo ImageNet pretrain direktno
        # Nema modifikacije prvog conv sloja
...
class ESANet(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # Razdvoji RGB i HHA (svaki po 3 kanala)
        rgb   = x[:, :3, :, :]   # [B, 3, H, W] — RGB
        depth = x[:, 3:6, :, :]  # [B, 3, H, W] — HHA
...
def build_esanet(
    ...
    """
    # in_channels moze biti 4 (RGB+depth) ili 6 (RGB+HHA)
