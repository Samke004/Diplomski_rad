...
def log_to_csv(save_path, config_name, epoch, stats):
    csv_file = os.path.join(save_path, f"metrics_log.csv")
    model_name = config_name.split('.')[-1] # npr. DFormer_Small
    
    fieldnames = [
        'epoch', 'model', 'overall_accuracy', 
        'pixel_accuracy_fg', 'frequency_weighted_iou', 'frequency_weighted_iou_fg',
        'mean_iou', 'mean_boundary_f1', 'branch_recall', 
        'iou_background', 'iou_trunk', 'iou_branches', 'iou_potpora'
    ]
    ...
    with open(csv_file, 'a', newline='') as f:
        ...
        ious = stats['ious']
        recalls = stats['cl_recall']
        
        writer.writerow({
            'epoch': epoch,
            'model': model_name,
            'overall_accuracy': round(stats['overall_acc'], 2),
            'pixel_accuracy_fg': round(stats['pixel_acc_fg'], 2),
            'frequency_weighted_iou': round(stats['fwiou'], 2),
            'frequency_weighted_iou_fg': round(stats['fwiou_fg'], 2),
            'mean_iou': round(stats['miou'], 2),
            'mean_boundary_f1': 0, # Placeholder jer se ne računa iz matrice
            'branch_recall': round(recalls[2], 2) if len(recalls) > 2 else 0,
            'iou_background': round(ious[0], 2) if len(ious) > 0 else 0,
            'iou_trunk': round(ious[1], 2) if len(ious) > 1 else 0,
            'iou_branches': round(ious[2], 2) if len(ious) > 2 else 0,
            'iou_potpora': round(ious[3], 2) if len(ious) > 3 else 0,
        })
            ...
            if engine.distributed:
                with torch.no_grad():
                    ...
                    if engine.local_rank == 0:
                        ...
                        # --- NOVO: Izračun proširenih metrika i zapis u CSV ---
                        stats = metric.compute_stats()
                        log_to_csv(config.log_dir, args.config, epoch, stats)
                        # -----------------------------------------------------

                        # Zadržavamo originalne pozive radi kompatibilnosti i printanja
